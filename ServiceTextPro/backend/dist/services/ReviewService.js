...
class ReviewService {
    ...
    async initializeReviewTables() {
        ...
        `, (err) => {
          ...
          ON case_reviews(provider_id, created_at DESC)
        `, (err) => {
                    if (err)
                        reject(err);
                    else
          ...
          ON case_reviews(customer_id, created_at DESC)
        `, (err) => {
                    if (err)
                        reject(err);
                    else
    ...
    async createReview(reviewData) {
        try {
           ...
           WHERE id = ? AND customer_id = ? AND provider_id = ? AND status = 'closed'`, [reviewData.caseId, reviewData.customerId, reviewData.providerId], (err, row) => {
                    if (err)
                        reject(err);
                    else
                        resolve(!!row);
            ...
            const existingReview = await new Promise((resolve, reject) => {
                this.db.db.get('SELECT id FROM case_reviews WHERE case_id = ? AND customer_id = ? AND provider_id = ?', [reviewData.caseId, reviewData.customerId, reviewData.providerId], (err, row) => {
                    if (err)
                        reject(err);
                    else
                        resolve(!!row);
    ...
    async getProviderReviewStats(providerId) {
        try {
            ...
            AVG(CAST(would_recommend AS FLOAT)) as recommendation_rate
           FROM case_reviews 
    ...
    async updateProviderRating(providerId) {
        try {
            ...
            await new Promise((resolve, reject) => {
                this.db.db.run(`UPDATE service_providers 
