...
class SQLiteDatabase {
    ...
    async initializeTables() {
      ...
      -- Drop old chat_tokens table if it exists with wrong structure
    ...
    continueConversationCreation(data, userId, resolve, reject) {
        this._db.get(`SELECT id FROM marketplace_conversations 
       WHERE provider_id = ? AND customer_email = ?`, [data.providerId, data.customerEmail], (err, row) => {
            if (err) {
                reject(err);
                return;
            }
            ...
            ) VALUES (?, ?, ?, ?, ?)`, [conversationId, data.providerId, data.customerName, data.customerEmail, data.customerPhone], (err) => {
                    if (err) {
                        reject(err);
                    }
                    else {
      ...
      `;
                this._db.all(query, [userId, userId], (err, rows) => {
                    if (err) {
                        ...
                        rows?.forEach((row) => {
                            console.log('  -', row.conversation_type, 'conversation:', row.id, 'customer:', row.customer_name || row.customer_phone, 'messages:', row.message_count);
                        });
                        resolve(rows || []);
                    }
                });
