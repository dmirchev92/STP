...
export class ChatTokenService {
  ...
  async getCurrentUnusedToken(userId: string, spIdentifier: string): Promise<string | null> {
    try {
      const result = await new Promise<any[]>((resolve, reject) => {
        (this.database as any)._db.all(
          'SELECT token FROM chat_tokens WHERE sp_identifier = ? AND is_used = 0 AND expires_at > datetime("now") ORDER BY created_at DESC LIMIT 1',
          [spIdentifier],
          (err: any, rows: any[]) => {
            if (err) reject(err);
            else resolve(rows || []);
  ...
  async generateNewToken(userId: string, spIdentifier: string): Promise<string> {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        (this.database as any)._db.run(
          'INSERT INTO chat_tokens (id, user_id, sp_identifier, token, is_used, expires_at) VALUES (?, ?, ?, ?, 0, datetime("now", "+7 days"))',
  ...
  async validateAndUseToken(spIdentifier: string, token: string): Promise<{
    ...
    try {
      ...
      const tableCheck = await new Promise<any[]>((resolve, reject) => {
        (this.database as any)._db.all(
          "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('chat_tokens', 'service_provider_identifiers')",
          (err: any, rows: any[]) => {
            if (err) reject(err);
            else resolve(rows || []);
          }
      ...
      logger.info('Available tables', { tables: tableCheck.map(t => t.name) });

      // Check table structure first
      const tableInfo = await new Promise<any[]>((resolve, reject) => {
        (this.database as any)._db.all(
          "PRAGMA table_info(chat_tokens)",
          (err: any, rows: any[]) => {
            if (err) reject(err);
            else resolve(rows || []);
          }
      ...
      logger.info('chat_tokens table structure', { columns: tableInfo.map(col => col.name) });

      // Try a simple query first
      const allTokens = await new Promise<any[]>((resolve, reject) => {
        (this.database as any)._db.all(
          "SELECT COUNT(*) as count FROM chat_tokens",
          (err: any, rows: any[]) => {
            if (err) reject(err);
            else resolve(rows || []);
          }
      ...
      logger.info('Total tokens in database', { count: allTokens[0]?.count || 0 });
      ...
      await new Promise<void>((resolve, reject) => {
        (this.database as any)._db.run(
          'UPDATE chat_tokens SET is_used = 1, used_at = datetime("now"), conversation_id = ? WHERE sp_identifier = ? AND token = ?',
      ...
      await new Promise<void>((resolve, reject) => {
        (this.database as any)._db.run(
          'INSERT INTO marketplace_conversations (id, provider_id, customer_name, customer_email, customer_phone, status, created_at, last_message_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"), datetime("now"))',
  ...
  async validateSession(sessionId: string): Promise<{
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        (this.database as any)._db.run(
          'UPDATE chat_sessions SET last_accessed_at = datetime("now") WHERE session_id = ?',
          [sessionId],
          (err: any) => {
            if (err) reject(err);
            else resolve();
   ...
   */
  async getChatUrlForUser(userId: string, baseUrl: string = 'http://192.168.0.129:3002'): Promise<string> {
  ...
  async forceRegenerateToken(userId: string): Promise<{ newToken: string; chatUrl: string }> {
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        (this.database as any)._db.run(
          'UPDATE chat_tokens SET is_used = 1, used_at = datetime("now") WHERE user_id = ? AND sp_identifier = ? AND is_used = 0',
          [userId, spIdentifier],
          (err: any) => {
            if (err) reject(err);
            else resolve();
        ...
        (this.database as any)._db.run(
          `DELETE FROM chat_tokens 
           WHERE expires_at < datetime("now") OR 
                 (is_used = 1 AND used_at < datetime("now", "-1 day"))`,
          (err: any) => {
            if (err) reject(err);
            else resolve({changes: (this.database as any)._db.changes || 0});
          }
        );
      });
