...
const EditProfileScreen: React.FC = () => {
  ...
  const loadProfileData = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        try {
          const providerResponse = await fetch(`http://192.168.0.129:3000/api/v1/marketplace/providers/${userData.id}`, {
            headers: {
  ...
  const handleImageUpload = async () => {
    try {
      ...
      // Upload image
      const uploadResponse = await fetch('http://192.168.0.129:3000/api/v1/uploads/image', {
        method: 'POST',
        headers: {
      ...
      if (uploadResult.success) {
        const baseUrl = 'http://192.168.0.129:3000';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updateResponse = await fetch('http://192.168.0.129:3000/api/v1/marketplace/providers/profile', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${await getStoredToken()}`,
