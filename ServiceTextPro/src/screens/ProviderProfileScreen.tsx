          ...
          <TouchableOpacity style={styles.headerAction} onPress={async () => {
            try {
              ...
              Alert.prompt?.('Поставете base64 изображение', undefined, async (base64) => {
                if (!base64) return;
                const res = await fetch('http://192.168.0.129:3000/api/v1/uploads/image', {
                  method: 'POST', headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ userId, filename: 'avatar.jpg', data: base64 })
                });
                const json = await res.json() as any;
                if (json.success) {
                  setProfileUrl(`http://192.168.0.129:3000${json.data.url}`);
                  setForm({ ...form, profileImageUrl: `http://192.168.0.129:3000${json.data.url}` });
                } else {
        ...
        <TouchableOpacity style={styles.secondaryButton} onPress={async () => {
          try {
            ...
            Alert.prompt?.('Поставете base64 изображение', undefined, async (base64) => {
              ...
              const json = await res.json() as any;
              if (json.success) {
                const url = `http://192.168.0.129:3000${json.data.url}`;
