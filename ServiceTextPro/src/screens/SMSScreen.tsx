...
function SMSScreen() {
  ...
  useEffect(() => {
    ...
    // Set up auto-refresh every 10 seconds to check for token updates
    const interval = setInterval(async () => {
      console.log('🔄 Auto-refreshing token from backend...');
      try {
        // Only refresh the token and display text, not the entire SMS data
  ...
  const loadSMSData = async () => {
    try {
      ...
      // Get config synchronously (no network calls)
