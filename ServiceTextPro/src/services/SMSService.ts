...
export class SMSService {
  ...
  private async generateNewChatLinkForUser(userId: string): Promise<void> {
    try {
      ...
      try {
        ...
        const pubResp = await fetch(`http://192.168.0.129:3000/api/v1/users/${actualUserId}/public-id`, { method: 'GET' });
        if (pubResp.ok) {
      ...
      const chatUrl = publicId
        ? `http://192.168.0.129:3002/u/${publicId}/c/${chatToken}`
        : `http://192.168.0.129:3002/c/${chatToken}`;
  ...
  private async forceRegenerateTokenForUser(userId: string): Promise<void> {
    try {
      ...
      if (authToken) {
        const response = await fetch('http://192.168.0.129:3000/api/v1/chat/tokens/regenerate', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`
      ...
      if (userId.startsWith('device_')) {
        ...
        const response = await fetch('http://192.168.0.129:3000/api/v1/chat/tokens/regenerate-device', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  public async sendMissedCallSMS(phoneNumber: string, callId: string, userId?: string): Promise<boolean> {
    ...
    if (!actualUserId) {
      try {
        const currentUserResponse = await fetch('http://192.168.0.129:3000/api/v1/auth/me', {
          method: 'GET',
  ...
  private async generateChatLinkMessage(userId: string): Promise<string> {
    ...
    const chatToken = this.generateShortSecureToken();
    const chatUrl = `http://192.168.0.129:3002/c/${chatToken}`;
...
${chatUrl}`;
  ...
  private async storeChatTokenMapping(token: string, userId: string): Promise<void> {
    try {
      ...
      const response = await fetch('http://192.168.0.129:3000/api/v1/chat/tokens', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  private async getUserPublicId(userId: string): Promise<string> {
    try {
      const response = await fetch(`http://192.168.0.129:3000/api/v1/users/${userId}/public-id`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  private async getCurrentUserIdAsync(): Promise<string | null> {
    try {
      ...
      const response = await fetch('http://192.168.0.129:3000/api/v1/auth/me', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`
  ...
  private async initializeChatTokenSystem(userId: string): Promise<{token: string, chatUrl: string} | null> {
    try {
      ...
      // Try authenticated endpoint first
      if (authToken) {
        const response = await fetch('http://192.168.0.129:3000/api/v1/chat/tokens/initialize', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`
      ...
      if (userId.startsWith('device_')) {
        ...
        const response = await fetch('http://192.168.0.129:3000/api/v1/chat/tokens/initialize-device', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  public async testGenerateChatLink(userId: string, phoneNumber: string): Promise<{ message: string; token: string; url: string }> {
    try {
      ...
      if (tokenData) {
        ...
        chatUrl = `http://192.168.0.129:3002/c/${chatToken}`;
      }
