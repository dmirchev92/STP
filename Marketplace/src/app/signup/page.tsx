...
export default function SignupPage() {
  ...
  const validateReferralCode = async (code: string) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/referrals/validate/${code}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const signupResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/auth/register`, {
        method: 'POST',
        headers: {
      ...
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/marketplace/providers/profile`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (referralCode && referralValid) {
        try {
          ...
          const referralResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/referrals/create`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
