...
export default function SettingsPage() {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/marketplace/providers/profile`, {
        method: 'POST',
        headers: {
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            reader.onloadend = async () => {
                              ...
                              try {
                                // Upload image
                                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/uploads/image`, {
                                  method: 'POST',
                                  headers: {
                                    'Content-Type': 'application/json',
                                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
                                ...
                                if (result.success) {
                                  ...
                                  const baseUrl = (process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1').replace('/api/v1', '')
