...
export default function SMSSettingsPage() {
  ...
  const loadSMSConfig = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/sms/config`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          setPreviewText(result.data.preview || result.data.config.message)
  ...
  const handleClearHistory = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/sms/history/clear`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
          'Content-Type': 'application/json'
