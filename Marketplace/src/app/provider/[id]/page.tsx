...
export default function ProviderDetailPage() {
  ...
  const updateProviderRating = async (providerId: string) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://192.168.0.129:3000/api/v1'}/reviews/provider/${providerId}/update-rating`, {
        method: 'POST',
        headers: {
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 shadow-xl border border-white/20">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-200">{(provider.rating || 0).toFixed(1)}/5</span>
              </div>
            </div>
          </div>
